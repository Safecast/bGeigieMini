 ...
 */
uint8_t sd_raw_read_interval(USB_ClassInfo_MS_Device_t* const MSInterfaceInfo, offset_t offset, uint8_t* buffer, uintptr_t interval, uintptr_t length, sd_raw_read_interval_handler_t callback, void* p)
{
  ...
  while(length >= interval)
  {
    ...
    if(!sd_raw_read(offset, buffer, interval))
      return 0;
    if(!callback(MSInterfaceInfo, buffer, offset, p))
      break;
    offset += interval;
    length -= interval;
  }
...
uint8_t sd_raw_write(offset_t offset, const uint8_t* buffer, uintptr_t length)
{
  ...
  while(length > 0)
  {
    ...
    // to transmit. Rounding up, this roughly gives a total of 13ms per 512 bytes block.
    // Set the watchdog timer accordingly to 30ms (to have little bit of slack)
    wdt_enable(WDTO_120MS); // writing seems to take longer than reading
 ...
 */
uint8_t sd_raw_write_interval(USB_ClassInfo_MS_Device_t* const MSInterfaceInfo, offset_t offset, uint8_t* buffer, uintptr_t length, sd_raw_write_interval_handler_t callback, void* p)
{
  if(!buffer || !callback)
    return 0;
  ...
  {
    uint16_t bytes_to_write = callback(MSInterfaceInfo, buffer, offset, p);
    if(!bytes_to_write)
      break;
